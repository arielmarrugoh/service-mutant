...
@Service
public class ReverseDiagonalSequenceValidation implements ValidationStrategy {
    ...
    public ResultValidate validate(String[] dnas) {
        ...
        resultValidate = reverseDiagonalSequenceValidationDesc(resultValidate, matrizChar, columnsSize, rowsSize);
        resultValidate = reverseDiagonalSequenceValidationAsc(resultValidate, matrizChar, columnsSize, rowsSize);

        return resultValidate;
    }

    private ResultValidate reverseDiagonalSequenceValidationDesc(ResultValidate resultValidate,
                                             char[][] matrizChar,
                                             int columnsSize,
                                             int rowsSize) {
        for(int row = 0; row< rowsSize; row++){
            StringBuilder sequenceVertical = new StringBuilder();
            for (int j = columnsSize-1, i = row; j >= 0 && i< rowsSize; i++,j--) {
    ...
    private ResultValidate reverseDiagonalSequenceValidationAsc (ResultValidate resultValidate,
                                                          char[][] matrizChar,
                                                          int columnsSize,
                                                          int rowsSize) {
        for(int colum = columnsSize-2; colum>= 0; colum--){
            StringBuilder sequenceVertical = new StringBuilder();
            for (int j = colum, i = 0; j >= 0 && i< rowsSize; i++,j--) {
                sequenceVertical.append(matrizChar[i][j]);
